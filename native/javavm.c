...
static int create_vm (lua_State *L) {
	...
	if (!(luastate_class = (*env)->FindClass(env, "org/terasology/jnlua/LuaState"))
	...
	if (!(library_class  = (*env)->FindClass(env, "org/terasology/jnlua/LuaState$Library"))
			|| !(openlib_id = (*env)->GetMethodID(env, luastate_class, "openLib", "(Lorg/terasology/jnlua/LuaState$Library;)V"))
			|| !(java_id = (*env)->GetStaticFieldID(env, library_class, "JAVA", "Lorg/terasology/jnlua/LuaState$Library;"))
